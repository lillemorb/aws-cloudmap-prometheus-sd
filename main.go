...
var (
	...
	outputFilePathFlag    = a.Flag("output.path", "Path for output file, without trailing /").Default("/output").String()
...
func main() {
	...
	sdAdapter := adapter.NewAdapter(ctx, *outputFileFlag, *outputFilePathFlag, "aws-cloudmap-prometheus-sd", disc, logger)
	sdAdapter.Run()
